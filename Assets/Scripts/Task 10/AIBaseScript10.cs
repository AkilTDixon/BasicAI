...
public class AIBaseScript10 : MonoBehaviour
{
    public enum AIType
    {
        Hero,
    ...
    void Update()
    {
        ...
        if (type == AIType.Hero || type == AIType.Prisoner)
        {
            ...
            if (target)
            {
                ...
                if (type == AIType.Hero)
                {
                    ...
                    HeroScript10 hs = GetComponent<HeroScript10>();

                    if (!hs.victory)
                    {
                        if (hs.detected && !hs.escape)
                        {
                            hs.avoidance = false;
                            GameObject closestGuardian = hs.GetClosestGuardian();
                            //change target to the guardian
                            //flee from the new target
                            if (hs.strat == HeroScript10.Strategy.Flee)
                            {
                                Flee(closestGuardian, sp);

                            }
                            else if (hs.strat == HeroScript10.Strategy.Destroy)
                            {
                                ...
                                {
                                    sp = closestGuardian.gameObject.GetComponent<AIBaseScript10>().maxSpeed;
                            ...
                            else if (hs.strat == HeroScript10.Strategy.Hunt)
                            {
                                if (hs.HoldingObjects.Count == 0)
                                {
                                    if (closestGuardian)
                                    {
                                        GuardianScript10 gs = closestGuardian.GetComponent<GuardianScript10>();
                                        ...
                                        sp = closestGuardian.gameObject.GetComponent<AIBaseScript10>().maxSpeed / distToCapture;
        ...
        {
            GuardianScript10 gs = GetComponent<GuardianScript10>();
            Velocity.y = 0;
    ...
    public void Pathfind(GuardianScript10 gs)
    {
        if (gs.foundPath)
        {
    ...
    private void Pathfind(HeroScript10 hs)
    {
        if (hs.foundPath)
        {
            RaycastHit hit;
            if (pathCounter < hs.pathPoints.Count - 1)
            {
                ...
                if ((hs.pathPoints[pathCounter] - transform.position).magnitude <= 3f)
                {
            ...
            {
                if ((hs.pathPoints[hs.pathPoints.Count - 1] - transform.position).magnitude <= 1f)
                {
                    pathCounter = 0;
                    hs.foundPath = false;
                    return;
