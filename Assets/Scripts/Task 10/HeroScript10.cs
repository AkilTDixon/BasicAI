...
public class HeroScript10 : MonoBehaviour
{
    ...
    [SerializeField] public float pathCheckInterval = 5f;
    ...
    public void OnCollisionEnter(Collision collision)
    {
        ...
        {
            /*SawScript8 ss = collision.gameObject.GetComponent<SawScript8>();
            if (collision.gameObject.transform.position.y == ss.maxY || collision.gameObject.transform.position.y == ss.minY)*/
            if (HoldingObjects.Count > 0)
            {
                for (int i = 0; i < HoldingObjects.Count; i++)
                {
                    HoldingObjects[i].GetComponent<PrisonerScript10>().Unfollow();
                    HoldingObjects.RemoveAt(i);
                }
                GameInfoScript.Instance.HeroCaptured2(gameObject);
            }
        }
    }
    public void OnTriggerEnter(Collider other)
    {
        ...
        {
            other.GetComponent<PrisonerScript10>().Follow();
            //pf.debugToggle = true;
        ...
        if (other.tag == "Base")
        {
            ...
            if (HoldingObjects.Count > 0)
            {
                ...
                for (int i = 0; i < HoldingObjects.Count; i++)
                {
                    ...
                    GameInfoScript.Instance.PrisonerSaved(HoldingObjects[i]);
                    //PrisonerObjects.Remove(HoldingObjects[i]);
                    HoldingObjects[i].GetComponent<PrisonerScript10>().desGuardians();
            ...
            else
            {
                if (!escape)
                {
                    if (PrisonerObjects.Count > 0 && HoldingObjects.Count == 0 && !detected)
                    {
                        if (agent.target.tag != "Prisoner")
                            GetClosestPrisoner();
                    }
                }
            }
    ...
    public void GetPathToBase()
    {
        ...
        pathPoints.Clear();
        agent.pathCounter = 0;
        foundPath = pf.Prelim(agent.target.transform.position, out pathPoints, false);
        pathPoints.Add(BasePoint.transform.position);

        Vector3 d = (pathPoints[pathPoints.Count - 1] - pathPoints[pathPoints.Count - 2]);
        if ((BasePoint.transform.position - (BasePoint.transform.position + d * multiplier)).magnitude < 10f)
            multiplier *= 1000f;
        pathPoints.Add(BasePoint.transform.position + d * multiplier);
    }
    // Start is called before the first frame update
    void Start()
    {
        pathCheckTimer = Time.time;
        h = new Helper();
    ...
    public void GetClosestPrisoner()
    {
        ...
        {
            //GameInfoScript.Instance.GameOver("Task 10");
            agent.target = BasePoint;
            foundPath = true;
            Destroy(gameObject);
        ...
        {
            if (PrisonerObjects[i])
            {
                float dist = (transform.position - PrisonerObjects[i].transform.position).magnitude;
                if (!PrisonerObjects[i].GetComponent<PrisonerScript10>().following)
                {
                    if (shortest == -1)
                    {
                        shortest = dist;
    ...
    {
        /*
 The behaviour I want is for it to drop everything and run as far away as possible if a player is detected


 */
        Collider[] rangeChecks = Physics.OverlapSphere(transform.position, viewRadius, agent.playerMask);
        ...
        if (rangeChecks.Length > 0)
        {
            ...
            //If player is in the cone of view
            //if (!escape)
            //{
                if (Vector3.Angle(transform.forward, d) < viewAngle / 2f || (transform.position - target.position).magnitude <= 4f)
                {
                    escapeTime = Time.time;
                    if (wait)
                        wait = false;
                    //as long as a player is detected once inside the FoV, the hero should be in the escaping state
                    escape = true;



                    //to find a far away point to escape to
                    Vector3 escapeDir = (transform.position - target.position).normalized;
                    LayerMask temp = agent.blockedMask;
                    if (detected)
                    {
                        
                        Vector3 a = (transform.position - target.position).normalized;
                        a.y = 0;
                        Vector3 b = (transform.position - detectedBy[0].transform.position).normalized;
                        b.y = 0;
                        Vector3 c = a + b;

                        escapeDir = c.normalized;


                    }

                    escapeDir.y = 0;
/*                    Debug.DrawRay(transform.position, escapeDir, Color.yellow);
                    Debug.Break();*/
                    //pf.debugToggle = true;
                    Vector3 escapePoint = transform.position + escapeDir * 100f;
                    GameObject obj = Instantiate(dummyObject, escapePoint, transform.rotation);
                    pathPoints.Clear();
                    agent.pathCounter = 0;
                    //agent.target = escapePoint;
                    //pf.debugToggle = true;
                    foundPath = pf.Prelim(escapePoint, out pathPoints, false);
                    agent.blockedMask = temp;
        ...
        {
            if (Time.time > escapeTime + escapeTimer)
            {
                // no players detected in the nearby circle
                if (escape)
                {
                    ...
                    agent.pathCounter = 0;
                    foundPath = false;
                    if (HoldingObjects.Count > 0 || PrisonerObjects.Count == 0)
                        GetPathToBase();
                    else if (PrisonerObjects.Count > 0)
                        GetClosestPrisoner();
                }
            }
        }
    ...
    private void viewCheck()
    {
        ...
        if (playerCheck())
        {
            if (agent.pathCounter < pathPoints.Count)
            {
                ...
                {
                    Vector3 escapeDir = (transform.position - GameInfoScript.Instance.playerObject.transform.position).normalized;
                    escapeDir.y = 0;
                    Vector3 escapePoint = transform.position + escapeDir * 50f;
                    GameObject obj = Instantiate(dummyObject, escapePoint, transform.rotation);
                    pathPoints.Clear();
                    agent.pathCounter = 0;
                    //agent.target = escapePoint;
        ...
        /*if (Time.time > pathCheckTimer + pathCheckInterval)
        {
            if (!detected)
            {
                foundPath = false;
        ...
        if (agent.target)
        {
            if (agent.target != BasePoint && agent.target.tag != "Guardian")
            {
                if (HoldingObjects.Count == 0 && agent.target.GetComponent<PrisonerScript10>().following)
                {
                    foundPath = false;
                    pathPoints.Clear();
                    agent.pathCounter = 0;
        ...
        if (pathPoints.Count > 0 && agent.pathCounter < pathPoints.Count)
        {
        ...
        if (!foundPath && !detected)
        {
            //foundPath = pf.StartPath(agent.target.transform.position, out pathPoints);
            foundPath = pf.Prelim(agent.target.transform.position, out pathPoints, hunting);
        }
        ...
        if (rangeChecks.Length != 0)
        {
            ...
            if (visible.Count > 0)
            {
                ...
                if (!Physics.Raycast(transform.position, direction, dist, agent.blockedMask))
                {
                    ...
                    GuardianScript10 gs = visible[index].GetComponent<GuardianScript10>();
                    ...
                    if (rangeChecks.Length > 2 || strat == Strategy.Hunt)
                    {
                        ...
                        if ((transform.position - (seenGuardian.transform.position)).magnitude > 5f && (transform.position - (circlePoint)).magnitude > 1f)
                        {
            ...
            else
            {
                ...
                //No longer in the view radius but nearby
                if (HoldingObjects.Count == 0)
                {
                    if (agent.target.tag == "Guardian" && !detected)
                        GetClosestPrisoner();
                }
                else if (agent.target.tag == "Guardian")
                    GetPathToBase();
    ...
    public void saved(GameObject prisoner)
    {
        PrisonerObjects.Remove(prisoner);
    }
    public IEnumerator fleeState(GameObject g)
    {
        ...
        {
            detected = false;
            if (HoldingObjects.Count == 0 && !escape)
            {
                GetClosestPrisoner();
                foundPath = false;
                pathPoints.Clear();
