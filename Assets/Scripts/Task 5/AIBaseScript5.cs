...
public class AIBaseScript5 : MonoBehaviour
{
    public enum AIType
    {
        Hero,
    ...
    void Update()
    {
        if (type == AIType.Hero || type == AIType.Prisoner)
        {
            ...
            if (target)
            {
                ...
                if (type == AIType.Hero)
                {
                    ...
                    HeroScript5 hs = GetComponent<HeroScript5>();
                    ...
                    if (!hs.victory)
                    {
                        if (hs.detected)
                        {
                            ...
                            if (hs.strat == HeroScript5.Strategy.Flee)
                            {
                                Flee(closestGuardian, sp);

                            }
                            else if (hs.strat == HeroScript5.Strategy.Destroy)
                            {
                                ...
                                {
                                    sp = closestGuardian.gameObject.GetComponent<AIBaseScript5>().maxSpeed;
                                    targetPath = hs.GetBaseDirectionLine();

                                }
                            }
                            else if (hs.strat == HeroScript5.Strategy.Hunt)
                            {
                                if (closestGuardian)
                                {
                                    sp = closestGuardian.gameObject.GetComponent<AIBaseScript5>().maxSpeed;
        ...
        {
            GuardianScript5 gs = GetComponent<GuardianScript5>();
            Velocity.y = 0;
