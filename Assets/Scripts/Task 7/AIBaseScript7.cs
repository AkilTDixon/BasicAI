...
public class AIBaseScript7 : MonoBehaviour
{
    public enum AIType
    {
        Hero,
    ...
    void Update()
    {
        ...
        if (type == AIType.Hero || type == AIType.Prisoner)
        {
            ...
            if (target)
            {
                ...
                if (type == AIType.Hero)
                {
                    ...
                    HeroScript7 hs = GetComponent<HeroScript7>();
                    ...
                    if (!hs.victory)
                    {
                        if (hs.detected)
                        {
                            ...
                            if (hs.strat == HeroScript7.Strategy.Flee)
                            {
                                Flee(closestGuardian, sp);

                            }
                            else if (hs.strat == HeroScript7.Strategy.Destroy)
                            {
                                ...
                                {
                                    sp = closestGuardian.gameObject.GetComponent<AIBaseScript7>().maxSpeed;
                            ...
                            else if (hs.strat == HeroScript7.Strategy.Hunt)
                            {
                                if (hs.HoldingObjects.Count == 0)
                                {
                                    if (closestGuardian)
                                    {
                                        GuardianScript7 gs = closestGuardian.GetComponent<GuardianScript7>();
                                        ...
                                        sp = closestGuardian.gameObject.GetComponent<AIBaseScript7>().maxSpeed / distToCapture;
        ...
        {
            GuardianScript7 gs = GetComponent<GuardianScript7>();
            Velocity.y = 0;
    ...
    public void Pathfind(GuardianScript7 gs)
    {
        if (gs.foundPath)
        {
    ...
    private void Pathfind(HeroScript7 hs)
    {
        if (hs.foundPath)
        {
            RaycastHit hit;
