...
public class AIBaseScript8 : MonoBehaviour
{
    public enum AIType
    {
        Hero,
    ...
    void Update()
    {
        ...
        if (type == AIType.Hero || type == AIType.Prisoner)
        {
            ...
            if (target)
            {
                ...
                if (type == AIType.Hero)
                {
                    ...
                    HeroScript8 hs = GetComponent<HeroScript8>();
                    ...
                    if (!hs.victory)
                    {
                        if (hs.detected)
                        {
                            ...
                            if (hs.strat == HeroScript8.Strategy.Flee)
                            {
                                Flee(closestGuardian, sp);

                            }
                            else if (hs.strat == HeroScript8.Strategy.Destroy)
                            {
                                ...
                                {
                                    sp = closestGuardian.gameObject.GetComponent<AIBaseScript8>().maxSpeed;
                            ...
                            else if (hs.strat == HeroScript8.Strategy.Hunt)
                            {
                                if (hs.HoldingObjects.Count == 0)
                                {
                                    if (closestGuardian)
                                    {
                                        GuardianScript8 gs = closestGuardian.GetComponent<GuardianScript8>();
                                        ...
                                        sp = closestGuardian.gameObject.GetComponent<AIBaseScript8>().maxSpeed / distToCapture;
        ...
        {
            GuardianScript8 gs = GetComponent<GuardianScript8>();
            Velocity.y = 0;
    ...
    public void Pathfind(GuardianScript8 gs)
    {
        if (gs.foundPath)
        {
    ...
    private void Pathfind(HeroScript8 hs)
    {
        if (hs.foundPath)
        {
            RaycastHit hit;
            if (pathCounter != hs.pathPoints.Count - 1)
            {
                ...
                if ((hs.pathPoints[pathCounter] - transform.position).magnitude <= 2f)
                {
                    ...
                    {
                        if ((hs.pathPoints[pathCounter] - transform.position).magnitude <= 1f)
                            if (mag1 <= mag2)
                                pathCounter++;
                    }
                    else if (mag1 <= mag2)
