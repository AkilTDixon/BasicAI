...
public class HeroScript9 : MonoBehaviour
{
    ...
    [SerializeField] public AIBaseScript9 agent;
    ...
    public void OnCollisionEnter(Collision collision)
    {
        if (collision.gameObject.name == "Saw")
        {
            ...
            if (collision.gameObject.transform.position.y == ss.maxY || collision.gameObject.transform.position.y == ss.minY)
                GameInfoScript.Instance.HeroCaptured(gameObject);
        }
    }
    public void OnTriggerEnter(Collider other)
    {
        ...
        {
            other.GetComponent<PrisonerScript9>().Follow();
            //pf.debugToggle = true;
        ...
        if (other.tag == "Base")
        {
            ...
            if (HoldingObjects.Count > 0)
            {
                ...
                for (int i = 0; i < HoldingObjects.Count; i++)
                {
                    ...
                    HoldingObjects[i].GetComponent<PrisonerScript9>().desGuardians();
            ...
            {
                if (PrisonerObjects.Count > 0 && agent.target == BasePoint && HoldingObjects.Count == 0 && !detected)
                    GetClosestPrisoner();
            }
        }
    }
    ...
    void Start()
    {
        ...
        h = new Helper();
        pf = GetComponent<PathfinderScript9>();
    ...
    public void GetClosestPrisoner()
    {
        ...
        {
            //GameInfoScript.Instance.GameOver("Task 9");
            agent.target = BasePoint;
            foundPath = true;
            Destroy(gameObject);
        ...
        {   if (PrisonerObjects[i])
            {
                ...
                if (!PrisonerObjects[i].GetComponent<PrisonerScript9>().following)
                {
                    if (shortest == -1)
                    {
                        shortest = dist;
    ...
    private void viewCheck()
    {
        ...
        if (agent.target)
        {
            ...
            {
                if (HoldingObjects.Count == 0 && agent.target.GetComponent<PrisonerScript9>().following)
                {
                    foundPath = false;
                    pathPoints.Clear();
                    agent.pathCounter = 0;
        ...
        if (rangeChecks.Length != 0)
        {
            ...
            if (visible.Count > 0)
            {
                ...
                if (!Physics.Raycast(transform.position, direction, dist, agent.blockedMask))
                {
                    ...
                    GuardianScript9 gs = visible[index].GetComponent<GuardianScript9>();
